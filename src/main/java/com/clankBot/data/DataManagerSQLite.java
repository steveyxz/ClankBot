...
public class DataManagerSQLite {
    ...
    public DataManagerSQLite(String path) {

        System.out.println("Connecting to database " + path);

        try {

            if (DataManagerSQLite.class.getResource(path) == null) {
                throw new NullPointerException("Data file was null");
            }
            URL url = DataManagerSQLite.class.getResource(path);
            assert url != null;
            ...
            System.out.println("Unsuccessful: Error occurred");
            e.printStackTrace();
        }
    }
